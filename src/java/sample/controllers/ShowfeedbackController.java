 ...
 * @author dangk
 */
@WebServlet(name = "ShowfeedbackController", urlPatterns = {"/ShowfeedbackController"})
public class ShowfeedbackController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<FeedbackDTO> listFeedback = dao.getListFeedBack(search);
            if (listFeedback.size()>0) {
                request.setAttribute("LIST_FEEDBACK", listFeedback);
                url = SUCCESS;
            }
        } catch (Exception e) {
            log("Error at ShowfeedbackController:" + e.toString());
        } finally {
