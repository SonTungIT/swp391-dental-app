...
public class NewsDAO {
    ...
    public NewsDTO getAbout1() throws SQLException {
    ...
    public NewsDTO getExpect1() throws SQLException {
        ...
        try {
            conn = DBUtils.getConnection();
            if (conn != null) {
                String sql = "select  top 1 * from News where categoryId = 4 order by timepost desc";
                stm = conn.prepareStatement(sql);
                rs = stm.executeQuery();
                while (rs.next()) {
                    NewsDTO news = new NewsDTO(rs.getInt(1),
